...
def ask_user_for_number():
    ...
    while not done:
        francois_number_to_calculate = int(input('Please input the Francois Number you wish to calculate (>0, 0 to end): '))
        if francois_number_to_calculate >= 0:
            done = True
        else:
            print(f'{francois_number_to_calculate} is invalid, please try again.')
    
    assert francois_number_to_calculate >= 0
...
def main():
    ...
    while francois_number_to_calculate != 0:
